...
/**
 * Creates a cluster icon using the actual egg logos as the visual
 * The cluster displays the egg logos in their proportion (camembert style)
 */
function createClusterIconFromLogos(size: number, cageCount: number, noCageCount: number): string {
  ...
  // Calculate logo scale to fit in the circle
  const logoScale = (size * 0.8) / 129; // 129 is the original logo height, use 80% of cluster size

  // If all one type, show just that logo scaled to fit
  if (cageCount === 0) {
    return `
      <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
        <!-- Clip circle -->
        <defs>
          <clipPath id="clip-circle-${size}">
            <circle cx="${centerX}" cy="${centerY}" r="${radius}"/>
          </clipPath>
        </defs>
        
        <!-- Free egg logo clipped to circle -->
        <g clip-path="url(#clip-circle-${size})">
          <g transform="translate(${centerX - (103 * logoScale) / 2}, ${centerY - (129 * logoScale) / 2}) scale(${logoScale})">
            ${FREE_EGG_ICON}
          </g>
        </g>
  ...
  if (noCageCount === 0) {
    return `
      <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Cage egg logo clipped to circle -->
        <g clip-path="url(#clip-circle-${size})">
          <g transform="translate(${centerX - (103 * logoScale) / 2}, ${centerY - (129 * logoScale) / 2}) scale(${logoScale})">
            ${CAGE_EGG_ICON}
          </g>
  ...
  // Mixed case: Create split circle showing both logos
  // Calculate split angle
  const cagePercentage = cageCount / total;
  const splitAngle = cagePercentage * 360;

  // For split, we'll use a vertical or horizontal split for simplicity
  // If close to 50/50, use vertical split, otherwise use a pie slice
  const usePieSlice = Math.abs(cagePercentage - 0.5) > 0.15; // If not close to 50/50

  if (!usePieSlice) {
    // Vertical split (close to 50/50)
    return `
      <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <!-- Clip for left half (cage) -->
          <clipPath id="clip-left-${size}">
            <rect x="0" y="0" width="${centerX}" height="${size}"/>
            <circle cx="${centerX}" cy="${centerY}" r="${radius}"/>
          </clipPath>
          
          <!-- Clip for right half (free) -->
          <clipPath id="clip-right-${size}">
            <rect x="${centerX}" y="0" width="${centerX}" height="${size}"/>
            <circle cx="${centerX}" cy="${centerY}" r="${radius}"/>
          </clipPath>
        </defs>
        
        <!-- Cage egg logo (left half) -->
        <g clip-path="url(#clip-left-${size})">
          <g transform="translate(${centerX - (103 * logoScale) / 2}, ${centerY - (129 * logoScale) / 2}) scale(${logoScale})">
            ${CAGE_EGG_ICON}
          </g>
        </g>
        
        <!-- Free egg logo (right half) -->
        <g clip-path="url(#clip-right-${size})">
          <g transform="translate(${centerX - (103 * logoScale) / 2}, ${centerY - (129 * logoScale) / 2}) scale(${logoScale})">
            ${FREE_EGG_ICON}
          </g>
        </g>
  ...
  // Pie slice approach for non-50/50 splits
  const startAngle = -90; // Start at top
  const endAngle = startAngle + splitAngle;
  ...
  const largeArcFlag = splitAngle > 180 ? 1 : 0;

  return `
    <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <!-- Clip for pie slice (cage portion) -->
        <clipPath id="clip-cage-${size}">
          <path d="M ${centerX},${centerY} L ${x1},${y1} A ${radius},${radius} 0 ${largeArcFlag},1 ${x2},${y2} Z"/>
        </clipPath>
        
        <!-- Clip for remaining portion (free) -->
        <clipPath id="clip-free-${size}">
          <circle cx="${centerX}" cy="${centerY}" r="${radius}"/>
        </clipPath>
      </defs>
      
      <!-- Background: Free egg logo (full circle) -->
      <g clip-path="url(#clip-free-${size})">
        <g transform="translate(${centerX - (103 * logoScale) / 2}, ${centerY - (129 * logoScale) / 2}) scale(${logoScale})">
          ${FREE_EGG_ICON}
        </g>
      </g>
      ...
      <g clip-path="url(#clip-cage-${size})">
        <g transform="translate(${centerX - (103 * logoScale) / 2}, ${centerY - (129 * logoScale) / 2}) scale(${logoScale})">
          ${CAGE_EGG_ICON}
        </g>
      </g>
...
/**
 * Creates a custom cluster icon using the egg logos as the visual representation
 */
export function createClusterCustomIcon(cluster: MarkerCluster) {
  ...
  // Generate cluster icon using egg logos
  const clusterSVG = createClusterIconFromLogos(size, cageCount, noCageCount);

  // Create the HTML with logo-based cluster and count overlay
  const html = `
